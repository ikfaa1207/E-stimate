<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Create Assembly</h2>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('assemblies.store') }}" class="space-y-4">
                        @csrf
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="unit" name="unit" class="mt-1 block w-full" :value="old('unit')" required />
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        @foreach($items as $item)
                                            <tr>
                                                ...
                                                <td class="px-4 py-2">
                                                    <input type="number" step="0.0001" min="0" name="items[{{ $item->id }}]" value="{{ old('items.'.$item->id, 0) }}" data-unit-cost="{{ $item->unit_cost }}" class="border-gray-300 rounded-md shadow-sm w-32 qty-input">
                                                </td>
